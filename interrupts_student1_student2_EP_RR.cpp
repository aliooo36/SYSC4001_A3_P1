    ...
    std::sort(
        ...
        [](const PCB &first, const PCB &second){
            if (first.priority == second.priority){
                return false; // maintain insertion order for same priority (FIFO for RR)
            }
    ...
    while(!all_process_terminated(job_list) || job_list.empty()) {
        ...
        for(auto &process : list_processes) {
            if(process.arrival_time == current_time) {//check if the AT = current time
                ...
                // Check if new arrival has higher priority than running process (EP preemption)
        ...
        //////////////////////////SCHEDULER//////////////////////////////
        ExternalPriorityRR(ready_queue); // Sort by priority, maintain FIFO for same priority
